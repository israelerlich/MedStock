...
<div class="bg-white shadow sm:rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Editar Movimentação</h3>
        
        <form action="{{ route('product-movements.update', $productMovement) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="product_id" id="product_id" required
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}" {{ old('product_id', $productMovement->product_id) == $product->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="client_id" id="client_id" required
                        ...
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ old('client_id', $productMovement->client_id) == $client->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        @foreach(\App\Enums\MovementType::cases() as $type)
                            <option value="{{ $type->value }}" {{ old('type', $productMovement->type->value) == $type->value ? 'selected' : '' }}>
                                {{ $type->label() }}
                            </option>
                        @endforeach
                    </select>
