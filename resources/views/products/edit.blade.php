...
<div class="bg-white shadow sm:rounded-lg">
    <div class="px-4 py-5 sm:p-6">
        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Editar Produto</h3>
        ...
        <form action="{{ route('products.update', $product) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select name="hospital_id" id="hospital_id"
                        ...
                        @foreach($hospitals as $hospital)
                            <option value="{{ $hospital->id }}" {{ old('hospital_id', $product->hospital_id) == $hospital->id ? 'selected' : '' }}>
                                {{ $hospital->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="expires_at" id="expires_at" value="{{ old('expires_at', $product->expires_at ? $product->expires_at->format('Y-m-d') : '') }}"
